...
class AmazonSpider(RedisSpider):
    ...
    def __init__(self, spider_name=None, start_depth=None, crawl_depth=None, keyword=None, star_num_min_limit=None,
                 star_num_max_limit=None, star_min_limit=None, star_max_limit=None, price_min_limit=None,
                 price_max_limit=None):
        self.name = spider_name
        self.redis_key = self.name + ":start_urls"
        self.url_start_depth = int(start_depth)
        self.crawl_depth = int(crawl_depth)
        self.crawl_depth = self.crawl_depth - self.url_start_depth - 1
        if self.crawl_depth < 0:
            self.crawl_depth = 0
        self.keyword = keyword
        self.star_num_min_limit = int(star_num_min_limit)
        self.star_num_max_limit = int(star_num_max_limit)
        self.star_min_limit = float(star_min_limit)
        self.star_max_limit = float(star_max_limit)
        self.price_min_limit = float(price_min_limit)
        self.price_max_limit = float(price_max_limit)
        # 大类小类列表xpath
        self.xpath_plus = 'ul/'
        self.xpath_start = '//*[@id="zg_browseRoot"]/ul/' + self.url_start_depth * self.xpath_plus
        self.xpath_end = 'li/a/'
    ...
    def parse(self, response):
        ...
        items = []
        parent_name = response.xpath('//*[@id="zg_browseRoot"]/ul/' + (self.url_start_depth - 1) * self.xpath_plus + '\
        ...
        if self.crawl_depth == 0:
            ...
            xpath = self.xpath_start + self.xpath_end
    ...
    def next_parse(self, response):
        ...
        xpath = self.xpath_start + self.xpath_plus * current_depth + self.xpath_end
        ...
        if not level_url_list:
            ...
            yield scrapy.Request(url=response.url + next_page_url_end,
                                 ...
                                 callback=self.rank_parse)
        # 当前分类已到爬取深度
        elif current_depth == self.crawl_depth:
    ...
    def detail_parse(self, response):
        ...
        try:
            ...
            # 提前过滤
            if self.star_max_limit >= item['product_stars'] >= self.star_min_limit and \
                    self.price_max_limit >= price >= self.price_min_limit:
    ...
    def earliest_review_pasre(self, response):
        ...
        if self.star_num_max_limit >= item['star_num'] >= self.star_num_min_limit and \
                self.star_max_limit >= item['product_stars'] >= self.star_min_limit:
            price = float(re.findall(r'\d+.\d+', item['product_price'])[0])
            if self.price_max_limit >= price >= self.price_min_limit:
                yield item
    ...
    def meta_to_item(self, level_title=None, level_url=None, product_name=None, product_url=None,
        ...
        item["star_num_min_limit"] = self.star_num_min_limit
        item["star_num_max_limit"] = self.star_num_max_limit
        item["star_min_limit"] = self.star_min_limit
        item["star_max_limit"] = self.star_max_limit
        item["price_min_limit"] = self.price_min_limit
        item["price_max_limit"] = self.price_max_limit
        return item
